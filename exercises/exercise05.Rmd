...
```{r}
d.bank.org <- read.table("../data/banknot.dat")
...
```{r}
d.bank.org <- read.table("banknot.dat")
ml.banknot <- Mclust(d.bank.org [,-1])
plot(... , what="BIC")
ml.cluster <- Mclust(... , modelNames="..." , G= ...)
# Here modelNames is the choice of method for the structure of the covariance matrix (EEE to VVV) and G the number of clusters.
```
